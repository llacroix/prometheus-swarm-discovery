...
def test_good_labels():
    ...
    for key in new_labels.keys():
        assert key.startswith(labels_prefix) is True
...
def test_format_label():
    ...
    assert (
        format_label("container", "image")
        == "__meta_docker_container_label_image"
    )
    assert (
        format_label("container", "some key")
        == "__meta_docker_container_label_some_key"
    )
