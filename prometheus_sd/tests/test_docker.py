...
async def create_container(docker, image_name, pull=True):
    ...
    container = await docker.containers.create(
        config=config, name="test_container"
    )
...
async def create_service(docker, image_name, pull=True):
    ...
    config = {"ContainerSpec": {"Image": image_name, }}

    service = await docker.services.create(
        task_template=config, name="test_service",
    )
...
async def test_build_image(loop):
    docker = aiodocker.Docker()
    await docker.swarm.init()
    ...
    with open("context.tar.gz", "rb") as context:
        image = await docker.images.build(
            fileobj=context, tag="promsd:latest", encoding="gzip",
        )
    ...
    service = await docker.services.create(
        task_template=service_config, name="promsd-service", labels=labels
    )

    parser = get_parser()
    config = Config(parser, ["--out", "/tmp/services.json"], loop)
    ...
    await docker.services.create(
        task_template=service_config, name="promsd-service2", labels=labels2
    )
    ...
    assert (
        "__meta_docker_service_label_prometheus_enable"
        in configs3[0]["labels"]
    )
...
async def test_save_configs(loop):
    scrape_config = [
        {
            "labels": {"job": "job1", },
    ...
    config = Config(parser, args=["--out", file_out], loop=loop)
...
async def test_config_deinit(loop):
    # success = True
    parser = get_parser()
    file_out = "/tmp/services.json"
    config = Config(parser, args=["--out", file_out], loop=loop)
